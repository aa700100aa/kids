 ...
 */
class Theme_Smart_Custom_Fields_Controller_Option extends Theme_Smart_Custom_Fields_Controller_Base {
	...
	public function __construct() {
		parent::__construct();
		add_action( Theme_SCF_Config::PREFIX . 'custom-options-page', array( $this, 'save_option' ) );
		add_action( Theme_SCF_Config::PREFIX . 'custom-options-page', array( $this, 'custom_options_page' ) );
	}
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		wp_enqueue_style(
			Theme_SCF_Config::PREFIX . 'option',
			get_stylesheet_directory_uri(). '/functions/' . Theme_SCF_Config::NAME . '/css/option.css'
		);
	}
	...
	public function custom_options_page( $Option ) {
		$settings = Theme_SCF::get_settings( $Option );
		if ( !$settings ) {
			return;
		}
		?>
