 ...
 */
class Theme_Smart_Custom_Fields_Meta {
	...
	public function get( $key = '', $single = false ) {
		...
		if ( $key === Theme_SCF_Config::PREFIX . 'repeat-multiple-data' ) {
			return $meta;
		}

		$settings = Theme_SCF::get_settings( $this->object );
		if ( $key ) {
	...
	public function update( $key, $value, $prev_value = '' ) {
		$return = false;
		do_action( Theme_SCF_Config::PREFIX . '-before-save-' . $this->meta_type, $this->id, $key, $value );
		$is_valid = apply_filters( Theme_SCF_Config::PREFIX . '-validate-save-' . $this->meta_type, $this->id, $key, $value );
		if ( $is_valid ) {
		...
		do_action( Theme_SCF_Config::PREFIX . '-after-save-' . $this->meta_type, $this->id, $key, $value );
		return $return;
	}
	...
	public function save( array $POST ) {
		...
		$this->delete( Theme_SCF_Config::PREFIX . 'repeat-multiple-data' );

		if ( !isset( $POST[Theme_SCF_Config::NAME] ) ) {
			return;
		}
		...
		foreach ( $settings as $Setting ) {
			...
			foreach ( $groups as $Group ) {
				...
				foreach ( $fields as $Field ) {
					...
					if ( !isset( $POST[Theme_SCF_Config::NAME][$field_name] ) ) {
						continue;
					}
					...
					if ( $Group->is_repeatable() && $Field->get_attribute( 'allow-multiple-data' ) ) {
						$repeat_multiple_data_fields = $POST[Theme_SCF_Config::NAME][$field_name];
		...
		if ( $repeat_multiple_data ) {
			$this->update( Theme_SCF_Config::PREFIX . 'repeat-multiple-data', $repeat_multiple_data );
		}

		foreach ( $POST[Theme_SCF_Config::NAME] as $name => $values ) {
	...
	public function restore( $revision ) {
		...
		foreach ( $settings as $Setting ) {
			...
			foreach ( $fields as $Field ) {
				$field_name = $Field->get( 'name' );
				$this->delete( $field_name );
				$value = Theme_SCF::get( $field_name, $revision->ID );
				if ( is_array( $value ) ) {
					foreach ( $value as $val ) {
						if ( is_array( $val ) ) {
							foreach ( $val as $v ) {
		...
		$repeat_multiple_data = Theme_SCF::get_repeat_multiple_data( $revision );
		$repeat_multiple_data_name = Theme_SCF_Config::PREFIX . 'repeat-multiple-data';
