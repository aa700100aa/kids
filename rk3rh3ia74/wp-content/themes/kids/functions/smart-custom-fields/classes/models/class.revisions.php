 ...
 */
class Theme_Smart_Custom_Fields_Revisions {
	...
	public function __construct() {
		add_filter(
			'_wp_post_revision_field_' . Theme_SCF_Config::PREFIX . 'debug-preview',
	...
	public function wp_restore_post_revision( $post_id, $revision_id ) {
		...
		$Meta = new Theme_Smart_Custom_Fields_Meta( $post );
	...
	public function wp_insert_post( $post_id ) {
		...
		$settings = Theme_SCF::get_settings( get_post( $post_id ) );
		if ( !$settings ) {
			return;
		}
		...
		$Meta = new Theme_Smart_Custom_Fields_Meta( get_post( $post_id ) );
		$Meta->save( $_POST );
	}
	...
	public function get_post_metadata( $value, $post_id, $meta_key, $single ) {
		...
		if ( is_null( Theme_SCF::get_field( get_post( $post_id ), $meta_key ) ) ) {
			return $value;
		}
	...
	public function _wp_post_revision_fields( $fields ){
		$fields[Theme_SCF_Config::PREFIX . 'debug-preview'] = esc_html__( 'Smart Custom Fields', 'smart-custom-fields' );
		return $fields;
	}
	...
	public function edit_form_after_title() {
		printf(
			...
			Theme_SCF_Config::PREFIX . 'debug-preview'
		);
	}
	...
	public function wp_save_post_revision_check_for_changes( $check_for_changes, $last_revision, $post ) {
		...
		if ( isset( $_POST[Theme_SCF_Config::NAME] ) ) {
			...
			$serialized_send_data = $_POST[Theme_SCF_Config::NAME];
