 ...
 */
class Theme_Smart_Custom_Fields_Group {
	...
	public function __construct( $group_name = null, $repeat = false, array $_fields = array() ) {
		...
		foreach ( $_fields as $field_attributes ) {
			$Field = Theme_SCF::get_form_field_instance( $field_attributes['type'] );
	...
	public function display_options( $group_key ) {
		?>
		<div class="<?php echo esc_attr( Theme_SCF_Config::PREFIX . 'group-repeat' ); ?>">
			<div class="<?php echo esc_attr( Theme_SCF_Config::PREFIX . 'icon-handle' ); ?>"></div>
			<label>
				<input type="checkbox"
					name="<?php echo esc_attr( Theme_SCF_Config::NAME . '[' . $group_key . '][repeat]' ); ?>"
					value="true"
		...
		<table class="<?php echo esc_attr( Theme_SCF_Config::PREFIX . 'group-names' ); ?> <?php $this->add_hide_class( $this->is_repeatable() ); ?>">
			<tr>
				<th><?php esc_html_e( 'Group name', 'smart-custom-fields' ); ?><span class="<?php echo esc_attr( Theme_SCF_Config::PREFIX . 'require' ); ?>">*</span></th>
				<td>
					<input type="text"
						name="<?php echo esc_attr( Theme_SCF_Config::NAME . '[' . $group_key . '][group-name]' ); ?>"
						size="30"
						class="<?php echo esc_attr( Theme_SCF_Config::PREFIX . 'group-name' ); ?>"
