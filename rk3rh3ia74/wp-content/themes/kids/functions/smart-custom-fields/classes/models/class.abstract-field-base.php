 ...
 */
abstract class Theme_Smart_Custom_Fields_Field_Base {
	...
	public function __construct() {
		...
		add_filter(
			Theme_SCF_Config::PREFIX . 'field-select-' . $attributes['optgroup'],
			array( $this, 'field_select' )
		);
		$this->after_loaded();

		Theme_SCF::add_form_field_instance( $this );
	}
	...
	public function display_options( $group_key, $field_key ) {
		?>
		<tr>
			<th><?php esc_html_e( 'Name', 'smart-custom-fields' ); ?><span class="<?php echo esc_attr( Theme_SCF_Config::PREFIX . 'require' ); ?>">*</span></th>
			<td>
				<input type="text"
					name="<?php echo esc_attr( $this->get_field_name_in_setting( $group_key, $field_key, 'name' ) ); ?>"
					size="30"
					class="<?php echo esc_attr( Theme_SCF_Config::PREFIX . 'field-name' ); ?>"
		...
		<tr>
			...
			<td>
				<input type="text"
					...
					size="30"
					class="<?php echo esc_attr( Theme_SCF_Config::PREFIX . 'field-label' ); ?>"
					value="<?php echo esc_attr( $this->get( 'label' ) ); ?>"
				/>
			</td>
		</tr>
		<?php
		$fields = Theme_SCF::get_form_field_instances();
	...
	public function _display_field_options( $group_key, $field_key ) {
		?>
		<tr class="<?php echo esc_attr( Theme_SCF_Config::PREFIX . 'field-options' ); ?> <?php echo esc_attr( Theme_SCF_Config::PREFIX . 'field-options-' . $this->get_attribute( 'type' ) ); ?> hide">
			<td colspan="2">
				<table>
