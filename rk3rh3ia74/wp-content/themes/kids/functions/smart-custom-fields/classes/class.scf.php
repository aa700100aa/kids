...
class Theme_SCF {
	...
	protected static function get_meta( $object, $name ) {
		$Cache = Theme_Smart_Custom_Fields_Cache::getInstance();
	...
	protected static function get_value_by_field( $object, $Field, $is_repeatable ) {
		...
		if ( is_array( $repeat_multiple_data ) && isset( $repeat_multiple_data[$field_name] ) ) {
			...
			foreach ( $repeat_multiple_data[$field_name] as $repeat_multiple_key => $repeat_multiple_value ) {
				...
				$value = apply_filters( Theme_SCF_Config::PREFIX . 'validate-get-value', $value, $field_type );
		...
		// Other than that
			...
			$meta = apply_filters( Theme_SCF_Config::PREFIX . 'validate-get-value', $meta, $field_type );
		}
		return $meta;
	}
	...
	public static function get_default_value( $Field, $single = false ) {
		if ( !is_a( $Field, 'Theme_Smart_Custom_Fields_Field_Base' ) ) {
			if ( $single ) {
				return '';
			}
			return array();
		...
		if ( $Field->get_attribute( 'allow-multiple-data' ) ) {
			$choices = Theme_SCF::choices_eol_to_array( $choices );
			$default = Theme_SCF::choices_eol_to_array( $default );
	...
	public static function get_settings_posts( $object ) {
		...
		switch ( $Meta->get_meta_type() ) {
			case 'post' :
				$key = Theme_SCF_Config::PREFIX . 'condition';
				break;
			case 'user' :
				$key = Theme_SCF_Config::PREFIX . 'roles';
				break;
			case 'term' :
				$key = Theme_SCF_Config::PREFIX . 'taxonomies';
				break;
			case 'option' :
				$key = Theme_SCF_Config::PREFIX . 'options-pages';
				break;
			default :
				$key = '';
		}
	...
	public static function get_settings( $object ) {
		...
		$settings = apply_filters(
			Theme_SCF_Config::PREFIX . 'register-fields',
			$settings,
			$type,
			$id,
			$meta_type,
	...
	protected static function get_settings_for_post( $object, $settings_posts ) {
		...
		foreach ( $settings_posts as $settings_post ) {
			...
			$condition_post_ids_raw = get_post_meta(
				$settings_post->ID,
				Theme_SCF_Config::PREFIX . 'condition-post-ids',
				true
			);
			if ( $condition_post_ids_raw ) {
				...
				foreach ( $condition_post_ids_raw as $condition_post_id ) {
					...
					$Setting = Theme_SCF::add_setting( $settings_post->ID, $settings_post->post_title );
	...
	public static function get_repeat_multiple_data( $object ) {
		...
		$Meta = new Theme_Smart_Custom_Fields_Meta( $object );
		$_repeat_multiple_data = $Meta->get( Theme_SCF_Config::PREFIX . 'repeat-multiple-data', true );
	 ...
	 */
	public static function add_form_field_instance( Theme_Smart_Custom_Fields_Field_Base $instance ) {
	...
	public static function add_setting( $id, $title ) {
		return new Theme_Smart_Custom_Fields_Setting( $id, $title );
	}
	...
	public static function add_options_page( $page_title, $menu_title, $capability, $menu_slug, $icon_url = '', $position = null ) {
		...
		new Theme_Smart_Custom_Fields_Options_Page( $page_title, $menu_title, $capability, $menu_slug, $icon_url, $position );
		return $menu_slug;
	}
