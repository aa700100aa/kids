 ...
 */
class Theme_Smart_Custom_Fields_Field_Related_Terms extends Theme_Smart_Custom_Fields_Field_Base {
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		wp_enqueue_script(
			Theme_SCF_Config::PREFIX . 'editor-relation-taxonomies',
			get_stylesheet_directory_uri(). '/functions/' . Theme_SCF_Config::NAME . '/js/editor-relation-taxonomies.js',
			array( 'jquery' ),
			null,
			true
		);
		wp_localize_script( Theme_SCF_Config::PREFIX . 'editor-relation-taxonomies', 'smart_cf_relation_taxonomies', array(
			'endpoint' => admin_url( 'admin-ajax.php' ),
			'action'   => Theme_SCF_Config::PREFIX . 'relational-terms-search',
			'nonce'    => wp_create_nonce( Theme_SCF_Config::NAME . '-relation-taxonomies' )
		) );
	}
	...
	public function relational_terms_search() {
		check_ajax_referer( Theme_SCF_Config::NAME . '-relation-taxonomies', 'nonce' );
		$_terms = array();
		$args = array();
